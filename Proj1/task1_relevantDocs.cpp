...
vector<pair<int, int>> FindTopDocuments(
    ...
    const string& query)
{
    vector<pair<int, int>> all_documents;
    all_documents = FindAllDocuments(word_to_documents, stop_words, query);

    sort(all_documents.begin(), all_documents.end(), [](pair<int, int> x, pair<int, int>y) {return x.second > y.second; });
    vector<pair<int, int>> top_documents(all_documents.begin(), all_documents.begin() + MAX_RESULT_DOCUMENT_COUNT);
